...
# - OPTIONAL: Create Animations from plots to visualize norm evolution in each network (HTML or gif)
...
Simulation <- function(
        ...
        ## Stable Parameters
        r = 20,                           # number of runs per parameter configuration
        ...
        CreatePlots = FALSE,              # controls whether the function does create and save a plot of every network iteration (VERY Time consuming)
        AnimatePlots = FALSE,             # controls whether all iteration plots of all networks are summarized into a GIF/HTML showcasing network evolution (VERY time consuming)
        ...
        if(majority == minority |
          ...
          warning("Input for demographic and norm attributes must be different between the two groups")
          stop()
        ...
        source("./Functions/HTMLAnimator.R")
        source("./Functions/GIFAnimator.R")
        ...
        if(AnimatePlots != FALSE & CreatePlots == FALSE){
        ...
        # creating HTML animations from generated plots if desired
        if(AnimatePlots == "HTML"){
                ...
                for(i in 1:length(OutputNetworkList)){
                        ...
                        HTMLAnimator(OutputNetworkList[[i]],Slave_i = i)
